...
const ChapterForm = ({ initialData, courseId }: ChapterFormProps) => {
  ...
  const toggleCreathing = () => setIsCreating((current) => !current);
  ...
  return (
    <div className="relative mt-6 border bg-slate-100 rounded-md p-4">
      ...
      <div className="font-medium flex items-center justify-between">
        Course Chapters
        <Button variant={"outline"} onClick={toggleCreathing}>
          {isCreating ? (
            <>Cancel</>
          ) : (
            <>
        ...
        <div
          className={cn(
            ...
            !initialData.chapters.length &&
              "m-2 p-1 pl-4 bg-slate-200 font-medium md:text-lg rounded-md text-slate-500 italic"
          )}
        >
